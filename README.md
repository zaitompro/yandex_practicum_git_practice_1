...
git push -u origin branch_name # Отправка изменений в удалённый репозиторий для отсутствующей на этот момент в удалённом репозитории ветки
git push # Отправка изменений в существующую ветку в удалённом репозитории
git push --force # Отправка изменений в удалённый репозиторий с удалением имеющихся там коммитов, отсутствующих в локальном репозитории
git log # Просмотр подробного списка коммитов репозитория
git log --oneline # Просмотр упрощённого списка коммитов репозитория
git log --graph # Отрисовка схемы состояния репозитория
git log branch_name1 branch_name2 # Просмотр истории коммитов в разрезе сразу нескольких веток
...
git diff branch_name branch_name # Просмотр изменений между ветками
git diff branch_name commit_hash # Просмотр изменений между текущим коммитом ветки и заданным коммитом
git diff [commit_hash|HEAD|branch_name]~число # Просмотр изменеий между состоянием текущей ветки и заданным числом коммитов назад (от 1 до количества коммитов в репозитории)
git diff [commit_hash|HEAD|branch_name]~число [commit_hash|HEAD|branch_name]~число # Просмотр изменений между заданными состояниями репозитория
git clone giturl # Клонирование удалённого репозитория в локальное хранилище
git branch # Отображение списка веток репозитория
git branch -a # Отображение списка всех возможных веток из локального и удалённого репозиториев
git branch branch_name # Создание ветки в репозитории
git checkout branch_name # Переключение на указанную ветку
git checkout -b branch_name # Создание новой ветки и переключение на неё
git merge branch_name # Слияние указанной ветки с веткой текущего местонахождения
git merge --no-ff branch_name # Слияние указанной ветки с веткой текущего местонахождения без режима fast-forward
git merge --no-edit branch_name # Слияние указанной ветки с веткой текущего местонахождения без изменения описания слияния
git branch -D branch_name # Удаление указанной ветки
git branch -d branch_name # Безопасное удаление указанной ветки с предварительной проверкой того, что в ветке нет изменений, отсутствующих в основной ветке
```
